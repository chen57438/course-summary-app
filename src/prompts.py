...
2. 【讲稿字幕覆盖视图 (TXT)】: {transcript_section} (这是教授口述的细节、案例和解释；对于长讲座，内容覆盖课程的开头、中段和结尾，而不是只取开头)
3. 【字幕精读视图】: {study_view_section} (这是去掉明显口头噪音后、按课程时间线选取的教学内容)
...
## Learning Objectives
列出学生完成本节后应能理解、比较或应用的 3-5 项能力。这个模块只是一张“学完后你能做什么”的能力导航，不要把它写成整节课的完整内容清单；完整知识细节必须展开在 Key Concepts。
